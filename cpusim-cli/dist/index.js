"use strict";
var __createBinding = (this && this.__createBinding) || (Object.create ? (function(o, m, k, k2) {
    if (k2 === undefined) k2 = k;
    Object.defineProperty(o, k2, { enumerable: true, get: function() { return m[k]; } });
}) : (function(o, m, k, k2) {
    if (k2 === undefined) k2 = k;
    o[k2] = m[k];
}));
var __setModuleDefault = (this && this.__setModuleDefault) || (Object.create ? (function(o, v) {
    Object.defineProperty(o, "default", { enumerable: true, value: v });
}) : function(o, v) {
    o["default"] = v;
});
var __importStar = (this && this.__importStar) || function (mod) {
    if (mod && mod.__esModule) return mod;
    var result = {};
    if (mod != null) for (var k in mod) if (k !== "default" && Object.prototype.hasOwnProperty.call(mod, k)) __createBinding(result, mod, k);
    __setModuleDefault(result, mod);
    return result;
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.run = exports.control = void 0;
var events = require('events');
const control_lines = __importStar(require("./control_lines"));
const control_lines_1 = require("./control_lines");
/*
  optionally slices the result in the same manner as verilog
*/
function tobin(value, pad, left, right) {
    if (typeof (left) === "undefined")
        left = pad - 1;
    if (typeof (right) === "undefined")
        right = 0;
    let v = value.toString(2);
    v = v.padStart(pad, "0");
    v = v.slice((pad - left) - 1, pad - right);
    return v;
}
function tobinB(value) {
    return value ? "1" : "0";
}
var Flag;
(function (Flag) {
    Flag[Flag["Set"] = 0] = "Set";
    Flag[Flag["Keep"] = 1] = "Keep";
})(Flag || (Flag = {}));
var CInv;
(function (CInv) {
    CInv[CInv["CInv"] = 0] = "CInv";
    CInv[CInv["CStd"] = 1] = "CStd";
})(CInv || (CInv = {}));
var AMode;
(function (AMode) {
    AMode[AMode["Dir"] = 0] = "Dir";
    AMode[AMode["Reg"] = 1] = "Reg";
})(AMode || (AMode = {}));
function assemble(rom, locn, aluOp, t, a, b, condition, setFlags, // set=true
conditionInvert, // invert=true
amode, // direct=true, register=false
address, immed) {
    let strF = ""
        + " O:" + tobin(aluOp, 5)
        + " T:" + tobin(t, 5, 3, 0)
        + " A:" + tobin(a, 3)
        + " B:" + tobin(b, 4, 2, 0)
        + " C:" + tobin(condition, 4)
        + " F:" + tobinB(setFlags == Flag.Set)
        + " I:" + tobinB(conditionInvert == CInv.CInv)
        + " b:" + tobin(b, 4, 3, 3)
        + " t:" + tobin(t, 5, 4, 4)
        + " a:" + tobinB(amode == AMode.Dir)
        + " @:" + tobin(address, 16)
        + " #:" + tobin(immed, 8);
    //println("MC : " + strF)
    let str = strF.replaceAll(/\s.:/g, "");
    let ctrl = new Control(() => BigInt(parseInt(str, 2)));
    ctrl.dump();
    //  println("STORE @ " + locn +  " = " + strF)
    rom.set(locn, str);
    //  let ctrl1 = new Control(() => rom.getAt(locn))
    //  ctrl1.dump("!")
    return str;
}
function println(str) {
    console.log(str);
}
function xor(a, b) {
    return !!a !== !!b;
}
function dec2bin(dec, pad) {
    if (typeof (pad) === "undefined")
        pad = 8;
    let v = dec.toString(2);
    v = v.padStart(pad, "0"); // pad out the string to the reqd extent
    return v;
}
function big2bin(big, pad) {
    if (typeof (pad) === "undefined")
        pad = 8;
    let v = big.toString(2);
    v = v.padStart(pad, "0"); // pad out the string to the reqd extent
    return v;
}
/* like verilog bit selection left and right are inclusive */
function slice(dec, width, top, bot) {
    if (typeof (bot) === "undefined")
        bot = top;
    if (top >= width)
        throw new Error("slice upper bound " + top + " exceeds width " + width);
    var bin = big2bin(dec, (top - bot) + 1);
    bin = bin.padStart(width, "0"); // pad out the string to the reqd extent
    var part = bin.slice((bin.length - top) - 1, bin.length - bot);
    return parseInt(part, 2);
}
class Bus {
    name;
    devices = [];
    constructor(name) {
        this.name = name;
    }
    attach(...devices) {
        devices.forEach(d => {
            this.devices.push(d);
        });
    }
    get() {
        let activeDevice = this.devices.find(device => {
            let v = device.get();
            // println(this.name + " checking " + device.name + " = " + v)
            return typeof v !== "undefined";
        }); // find any device that returns a value
        let r = undefined;
        if (typeof activeDevice !== "undefined") {
            r = activeDevice.get();
            //println("BUS "+ this.name + " = " + r + " : " + activeDevice.name)
        }
        else {
            //println("BUS "+ this.name + " = " + r)
        }
        return r;
    }
}
class ALU {
    control;
    abus;
    bbus;
    constructor(control, carryBit, abus, bbus) {
        this.control = control;
        this.abus = abus;
        this.bbus = bbus;
    }
    get() {
        let ret = 0;
        const op = this.control.alu_op();
        let a = this.abus.get();
        let b = this.bbus.get();
        switch (+op) {
            case control_lines_1.Op.ZERO:
                ret = 0;
                break;
            case control_lines_1.Op.A:
                ret = a;
                break;
            case control_lines_1.Op.B:
                ret = b;
                break;
            case control_lines_1.Op.A_PLUS_B:
                ret = a + b;
                break;
            default: {
                println("alu op " + control_lines_1.Op[exports.control.alu_op()] + " not yet supported");
                throw new Error("alu op " + exports.control.alu_op() + " not yet supported");
            }
        }
        //println("ALU OUT " + ret)
        let carryOut = ret > 255;
        return ret & 0xff;
    }
}
class ROM {
    pc;
    name = "ROM";
    data1 = new Uint8Array(Math.pow(2, 16));
    data2 = new Uint8Array(Math.pow(2, 16));
    data3 = new Uint8Array(Math.pow(2, 16));
    data4 = new Uint8Array(Math.pow(2, 16));
    data5 = new Uint8Array(Math.pow(2, 16));
    data6 = new Uint8Array(Math.pow(2, 16));
    constructor(pc) {
        let f = (~0xaa & 0xff) + 2;
        this.pc = pc;
        this.data1.fill(f);
        this.data2.fill(f);
        this.data3.fill(f);
        this.data4.fill(f);
        this.data5.fill(f);
        this.data6.fill(f);
    }
    getRom() {
        let address = this.pc.get();
        return this.getAt(address);
    }
    get() {
        let address = this.pc.get();
        return this.data1[address];
    }
    getAt(address) {
        //println("get ROM[" + address + "]");
        let result = (BigInt(this.data6[address]) << (5n * 8n)) +
            (BigInt(this.data5[address]) << (4n * 8n)) +
            (BigInt(this.data4[address]) << (3n * 8n)) +
            (BigInt(this.data3[address]) << (2n * 8n)) +
            (BigInt(this.data2[address]) << (1n * 8n)) +
            (BigInt(this.data1[address]));
        return result;
    }
    immed() {
        let address = this.pc.get();
        return this.data1[address];
    }
    address() {
        let address = pc.get();
        return (this.data2[address] << 8) + this.data1[address];
    }
    // data is a 48 bit number
    set(locn, data) {
        if (data.length != 48)
            throw new Error("data must be 48 bits");
        let datas = data.match(/.{1,8}/g);
        this.data6[locn] = parseInt(datas[0], 2);
        this.data5[locn] = parseInt(datas[1], 2);
        this.data4[locn] = parseInt(datas[2], 2);
        this.data3[locn] = parseInt(datas[3], 2);
        this.data2[locn] = parseInt(datas[4], 2);
        this.data1[locn] = parseInt(datas[5], 2);
    }
}
class Control extends control_lines.Pins {
    rom;
    constructor(rom) {
        super();
        this.rom = rom;
    }
    romSlice(left, right) {
        if (typeof (right) === "undefined")
            right = left;
        return () => {
            return slice(this.rom(), 48, left, right);
        };
    }
    romSliceB(left) {
        return () => {
            return slice(this.rom(), 48, left, left) == 1;
        };
    }
    // instruction decoding - same names and slices as verilog for consistency
    immed = this.romSlice(7, 0);
    address = this.romSlice(23, 8);
    amode_bit = this.romSliceB(24);
    targ_dev_4 = this.romSlice(25);
    bbus_dev_3 = this.romSlice(26);
    cond_invert_bit = this.romSliceB(27);
    set_flags_bit = this.romSliceB(28);
    condition_bot = this.romSlice(31, 29);
    condition_top = this.romSlice(32);
    bbus_dev_2_0 = this.romSlice(35, 33);
    abus_dev = this.romSlice(38, 36);
    targ_dev_3_0 = this.romSlice(42, 39);
    alu_op = this.romSlice(47, 43);
    condition = () => (this.condition_top() << 4) + this.condition_bot();
    bbus_dev = () => (this.bbus_dev_3() << 3) + this.bbus_dev_2_0();
    targ_dev = () => (this.targ_dev_4() << 4) + this.targ_dev_3_0();
    condition_met = () => {
        let _flags = 0b0000000000000000; // TODO: all flags set
        let cond_flag_bit = 1 << this.condition();
        return (_flags & cond_flag_bit) == 0;
    };
    do_exec = () => {
        //println("COND INV "+ this.cond_invert_bit()) // xor
        return xor(this.condition_met(), this.cond_invert_bit());
    };
    set_flags = () => (this.set_flags_bit() && this.do_exec());
    addrmode_register = () => !this.amode_bit();
    addrmode_direct = () => this.amode_bit();
    // "one bit hot"
    // NOTE ! positive logic - the hardware is neg logic
    chk_t_dev(bit) {
        // println("TARG = " + this.targ_dev());
        let selected = (bit == this.targ_dev());
        if (this.do_exec()) {
            return selected;
        }
        else {
            return false;
        }
    }
    chk_a_dev(bit) {
        // println("ADEV = " + this.abus_dev());
        return bit == this.abus_dev();
    }
    chk_b_dev(bit) {
        // println("BDEV = " + this.bbus_dev());
        return bit == this.bbus_dev();
    }
    dump(prefix) {
        if (typeof prefix == "undefined")
            prefix = "";
        //   println("DUMP: " + prefix + " " + this.rom().toString(2).padStart(48,"0"))
        //   println(`DUMP: ${this.targ_dev()} = ${this.abus_dev()} (${this.alu_op()}) ${this.bbus_dev()} ex:${this.do_exec()} ci:${this.cond_invert_bit()} cond:${this.condition()}`)
        //   println(`DUMP: ${TDev[this.targ_dev()]} = ${ADev[this.abus_dev()]} (${Op[this.alu_op()]}) ${BDev[this.bbus_dev()]} ex:${this.do_exec()} ci:${this.cond_invert_bit()} cond:${this.condition()}`)
    }
}
class RAM {
    control;
    aluBus;
    mar;
    ram = new Array();
    name = "RAM";
    latchedValue;
    latchedAddress;
    constructor(control, aluBus, mar) {
        this.control = control;
        this.aluBus = aluBus;
        this.mar = mar;
        this.ram.fill(undefined);
        Object.seal(this.ram);
        clock.on(clock.EXEC1, () => this.latch());
        clock.on(clock.EXEC2, () => this.update());
    }
    latch() {
        this.latchedValue = this.aluBus();
        this.latchedAddress = this.mar.get();
    }
    update() {
        println("MARLO IN " + this.control.marlo_in()());
        println("RAM IN " + this.control.ram_in()());
        if (this.control.ram_in()()) {
            println(`LOADING RAM[${this.latchedAddress}] <- ${this.latchedValue}`);
            this.ram[this.latchedAddress] = this.latchedValue;
        }
    }
    get() {
        let address = this.mar.get();
        return this.ram[address];
    }
}
class Register {
    writeEnabled;
    outputEnabled;
    d;
    value = undefined;
    name;
    latchedValue;
    constructor(name, clock, writeEnabled, outputEnabled, d) {
        this.name = name;
        this.d = d;
        this.writeEnabled = writeEnabled;
        this.outputEnabled = outputEnabled;
        clock.on(clock.EXEC1, () => this.latch());
        clock.on(clock.EXEC2, () => this.update());
    }
    latch() {
        this.latchedValue = this.d();
    }
    update() {
        // println("CLK " + this.name + " : WE " + this.writeEnabled())
        if (this.writeEnabled()) {
            this.value = this.latchedValue;
            println("LOADING REG " + this.name + " <- " + this.value);
        }
    }
    get() {
        if (this.outputEnabled()) {
            //      println("REG " + this.name + " OUT ENABLED - OUT " + this.value);
            return this.value;
        }
        else {
            //      println("REG " + this.name + " OUT DISABLED");
            return undefined;
        }
    }
}
class DualPortRegister {
    a;
    b;
    constructor(name, clock, aluBus, we, oea, oeb) {
        this.a = new Register(name + "_a", clock, we, oea, aluBus);
        this.b = new Register(name + "_b", clock, we, oeb, aluBus);
    }
}
class MAR {
    lo;
    hi;
    constructor(clock, control, aluBus) {
        this.lo = new DualPortRegister("MARLO", clock, aluBus, control.marlo_in(), control.adev_marlo(), control.bdev_marlo());
        this.hi = new DualPortRegister("MARHI", clock, aluBus, control.marhi_in(), control.adev_marhi(), control.bdev_marhi());
    }
    // 16 bit
    get() {
        const hi = this.hi.a.get() << 8;
        return hi + this.lo.a.get();
    }
}
class RegisterFile {
    reg;
    constructor(clock, control, aluBus) {
        this.reg = [
            new DualPortRegister("REGA", clock, aluBus, control.rega_in(), control.adev_rega(), control.bdev_rega()),
            new DualPortRegister("REGB", clock, aluBus, control.regb_in(), control.adev_regb(), control.bdev_regb()),
            new DualPortRegister("REGC", clock, aluBus, control.regc_in(), control.adev_regc(), control.bdev_regc()),
            new DualPortRegister("REGD", clock, aluBus, control.regd_in(), control.adev_regd(), control.bdev_regd()),
        ];
    }
}
class PC {
    control;
    aluBus;
    hitmp = 0;
    hi = 0;
    lo = 0;
    jumped = false;
    latchedPcLo;
    latchedPcHi;
    latchedPcHiTmp;
    constructor(clock, control, aluBus) {
        this.control = control;
        this.aluBus = aluBus;
        clock.on(clock.EXEC1, () => this.latch());
        clock.on(clock.EXEC2, () => this.update());
        clock.on(clock.INCPC, () => this.inc());
    }
    latch() {
        // latch existing values so that if no update occurs then we essentially do a no op
        this.latchedPcLo = this.lo;
        this.latchedPcHi = this.hi;
        this.latchedPcHiTmp = this.hitmp;
        this.jumped = false;
        var bus = this.aluBus();
        println("PC LATCHING : " + bus);
        if (this.control.pc_in()()) {
            this.latchedPcHi = this.hitmp;
            this.latchedPcLo = bus;
            this.jumped = true;
            println("PC LOADING HI:LO = " + this.hitmp + ":" + bus + " = " + this.get());
        }
        if (this.control.pclo_in()()) {
            this.latchedPcLo = bus;
            this.jumped = true;
            println("PC LOADING LO = " + bus);
        }
        if (this.control.pchitmp_in()()) {
            println("PC LOADING HITMP = " + bus);
            this.latchedPcHi = bus;
        }
    }
    update() {
        this.lo = this.latchedPcLo;
        this.hi = this.latchedPcHi;
        this.hitmp = this.latchedPcHiTmp;
        println(`PC updating ${this.lo} ${this.hi} ${this.hitmp}`);
    }
    get() {
        var address = (this.hi << 8) + this.lo;
        /*
        var err = new Error();
        println("PC STACK " + err.stack);
        println("PC RETURNING " + address)
        println("PC HI " + this.hi)
        println("PC LO " + this.lo)
        */
        return address;
    }
    cycles = 0;
    inc() {
        if (!this.jumped) {
            var newPc = this.get() + 1;
            println("PC ADVANCE : PC = " + newPc + "      Cycles = " + this.cycles);
            this.lo = newPc & 0xff;
            this.hi = (newPc >> 8) & 0xff;
            this.cycles++;
        }
    }
}
function tohex(v) {
    if (typeof (v) === "undefined")
        return "undef";
    return v.toString(16).padStart(2, "0");
}
// Wiring and Clocking
class Clock extends events.EventEmitter {
    EXEC1 = "EXEC1";
    EXEC2 = "EXEC2";
    INCPC = "INCPC";
    halted = false;
    cycle() {
        println("");
        exports.control.dump();
        println("--- EXEC LATCH");
        clock.emit(this.EXEC1);
        println("--- EXEC PROPAGATE");
        clock.emit(this.EXEC2);
        if (exports.control.halt_in()()) {
            println("!!! HALTING !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!");
            println(`!!! PC   HI:LO ${tohex(pc.hi)}:${tohex(pc.lo)} = ${pc.get()}`);
            println(`!!! MAR  HI:LO ${tohex(mar.hi.a.value)}:${tohex(mar.lo.a.value)} = ${mar.lo.a.value}`);
            println(`!!! REG      A ${tohex(regfile.reg[0].a.get())} = ${regfile.reg[0].a.get()}`);
            println(`!!! REG      B ${tohex(regfile.reg[1].a.get())} = ${regfile.reg[1].a.get()}`);
            println(`!!! REG      C ${tohex(regfile.reg[2].a.get())} = ${regfile.reg[2].a.get()}`);
            println(`!!! REG      D ${tohex(regfile.reg[3].a.get())} = ${regfile.reg[3].a.get()}`);
            this.halted = true;
        }
        println("--- INCPC");
        clock.emit(this.INCPC);
    }
}
const clock = new Clock();
clock.on(Clock.EXEC, () => println("CLOCK UP "));
const bBus = new Bus("bBus");
const aBus = new Bus("aBus");
let rom;
println("SETTING UP CONTROL");
exports.control = new Control(() => {
    if (typeof (rom) === "undefined") {
        println("warning : reading uninitialised rom");
        return 0n;
    }
    return rom.getRom();
});
println("SETTING UP ALU");
const alu = new ALU(exports.control, () => false, aBus, bBus);
println("SETTING UP PC");
const pc = new PC(clock, exports.control, () => alu.get());
println("SETTING UP ROM");
rom = new ROM(pc);
println("SETTING UP MAR");
const mar = new MAR(clock, exports.control, () => alu.get());
println("SETTING UP REGFILE");
const regfile = new RegisterFile(clock, exports.control, () => alu.get());
println("SETTING UP RAM");
const ram = new RAM(exports.control, () => alu.get(), mar);
println("ATTACHING BUS");
aBus.attach(mar.hi.a, mar.lo.a, regfile.reg[0].a, regfile.reg[1].a, regfile.reg[2].a, regfile.reg[3].a);
bBus.attach(mar.hi.b, mar.lo.b, regfile.reg[0].b, regfile.reg[1].b, regfile.reg[2].b, regfile.reg[3].b, ram, rom);
println("ASSEMBLING");
//  rom, locn, aluOp, t, a, b, condition, setFlags, conditionInvert, amode, address, immed
assemble(rom, 0, control_lines_1.Op.B, control_lines_1.TDev.marlo, control_lines_1.ADev.nu, control_lines_1.BDev.immed, control_lines_1.Cond.A, Flag.Set, CInv.CStd, AMode.Dir, 0, 0);
//assemble(rom, 1, Op.B,        TDev.marlo,   ADev.nu, BDev.immed, Cond.A, Flag.Set, CInv.CStd, AMode.Dir, 0, 0)
assemble(rom, 1, control_lines_1.Op.B, control_lines_1.TDev.marhi, control_lines_1.ADev.nu, control_lines_1.BDev.immed, control_lines_1.Cond.A, Flag.Set, CInv.CStd, AMode.Dir, 0, 0);
assemble(rom, 2, control_lines_1.Op.A_PLUS_B, control_lines_1.TDev.marlo, control_lines_1.ADev.marlo, control_lines_1.BDev.immed, control_lines_1.Cond.A, Flag.Set, CInv.CStd, AMode.Dir, 0, 1);
assemble(rom, 3, control_lines_1.Op.B, control_lines_1.TDev.pchitmp, control_lines_1.ADev.nu, control_lines_1.BDev.immed, control_lines_1.Cond.A, Flag.Set, CInv.CStd, AMode.Dir, 0, 0);
//assemble(rom, 4, Op.B,        TDev.pc,      ADev.nu, BDev.immed, Cond.C, Flag.Set, CInv.CStd, AMode.Dir, 0, 2)
assemble(rom, 4, control_lines_1.Op.B, control_lines_1.TDev.halt, control_lines_1.ADev.nu, control_lines_1.BDev.immed, control_lines_1.Cond.C, Flag.Set, CInv.CStd, AMode.Dir, 0, 0);
while (!clock.halted) {
    clock.cycle();
}
/*
aBus.get()
clock.cycle();
*/
println("-----");
println(`DEV_SELECT T:${exports.control.targ_dev()} B:1 A:2`); // .f(control.targ_dev(), control.bbus_dev(), control.abus_dev()))
println("TARG_DEV 12 = " + exports.control.chk_t_dev(12));
println("BBUS_DEV 1 = " + exports.control.chk_b_dev(1));
println("ABUS_DEV 0 = " + exports.control.chk_a_dev(0));
println("ABUS_DEV rega = " + exports.control.adev_rega()());
println("ABUS_DEV regb = " + exports.control.adev_regb()());
println("BBUS_DEV immed = " + exports.control.bdev_immed()());
function run() {
    throw new Error("run called -> DONE");
}
exports.run = run;
//# sourceMappingURL=index.js.map